...
def handle_pinterest(task: Dict[str, Any], temp_dir: str) -> bool:
    """Handles Pinterest video / photo / pin download."""
    url = task["url"]
    chat_id = task["chat_id"]
    thread_id = task["thread_id"]
    status_msg_id = task["status_msg_id"]
    ...
    logger.info(f"📌 Processing Pinterest: {url}")
    ts = int(time.time())
    title = f"Pinterest_{ts}"
    ...
    if chat_id and status_msg_id:
        telegram_helper.edit_message(
            chat_id, status_msg_id,
            f"📌 <b>[Cloud Runner: {RUNNER_REPO}] Đang tải Pinterest...</b>\n🔗 <code>{html.escape(url)}</code>"
        )
    ...
    # 2. Scraper fallback for Pinterest image pins
    if not downloaded:
        try:
            ...
            if og_v:
                ...
                img_url = og_i.group(1)
                orig_url = re.sub(r'/\d+x/', '/originals/', img_url)
                r_i = requests.get(orig_url, headers=headers, timeout=30)
                if r_i.status_code != 200:
                    r_i = requests.get(img_url, headers=headers, timeout=30)
                if r_i.status_code == 200:
                    ext = ".png" if ".png" in img_url.lower() else ".jpg"
                    i_f = os.path.join(temp_dir, f"{title}{ext}")
                    with open(i_f, "wb") as f:
                        f.write(r_i.content)
                    downloaded.append(i_f)
    ...
    if not downloaded:
        ...
        if chat_id and status_msg_id:
            telegram_helper.edit_message(chat_id, status_msg_id, f"❌ <b>Không thể tải Pinterest. Vui lòng kiểm tra link công khai.</b>")
        return False
